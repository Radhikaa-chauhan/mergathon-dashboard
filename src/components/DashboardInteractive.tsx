...
export default function DashboardInteractive() {
  ...
  const teams = useMemo(() => [...data.teams].sort((a, b) => b.totalScore - a.totalScore), [data.teams]);

  const totalScoreCombined = useMemo(() => teams.reduce((acc, t) => acc + t.totalScore, 0), [teams]);
  ...
  ], [stats.totalIssuesClosed, stats.totalPrsMerged]);

  const allContributions = useMemo(() => contributors
    .flatMap(c =>
    ...
    .slice(0, 6), [contributors]);

  return (
    <div>
      <div className="capsule-toggle">
        <button className={`toggle-option ${view === "overview" ? "active" : ""} ${isPending && view !== "overview" ? "pending" : ""}`} onClick={() => startTransition(() => setView("overview"))}>Overview</button>
        <button className={`toggle-option ${view === "teams" ? "active" : ""} ${isPending && view !== "teams" ? "pending" : ""}`} onClick={() => startTransition(() => setView("teams"))}>Teams</button>
      </div>
                  ...
                  <div key={team.name} style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "8px" }}>
                      ...
                      <Link href={`/teams/${encodeURIComponent(team.name)}`} style={{ textDecoration: "none" }}>
                        <span style={{ fontSize: "13px", fontWeight: 700, color: "var(--text-primary)", transition: "color 0.2s" }} className="clickable-team-title">
                          {team.name}
                        </span>
                      </Link>
                    </div>
                ...
                {teams.map((team) => (
                  <div key={team.name} style={{ background: "rgba(255, 255, 255, 0.01)", border: "1px solid var(--border-primary)", borderRadius: "var(--radius-lg)", padding: "16px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                      ...
                      <div>
                        <Link href={`/teams/${encodeURIComponent(team.name)}`} style={{ textDecoration: "none" }}>
                          <h4 style={{ fontSize: "16px", fontWeight: 800, color: "#ffffff", transition: "color 0.2s" }} className="clickable-team-title">
                            {team.name}
                          </h4>
                        </Link>
